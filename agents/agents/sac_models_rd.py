...
from agents.envs import RandomDelayEnv, UntouchedGymEnv
...
# Hybrid model for trackmania (Yann):  #TODO!!!!!!!!!!!!!!!!!!!!



class TMModule(Module):
    def __init__(self, observation_space, action_space, is_Q_network, hidden_units: int = 256):  # FIXME: action_space param is useless
        """
        Args:
        """
        super().__init__()
...
class Tm_hybrid_1(ActorModule):
    def __init__(self, observation_space, action_space, hidden_units: int = 256, num_critics: int = 2):
        super().__init__()
        assert isinstance(observation_space, gym.spaces.Tuple)
        self.critics = ModuleList(TMActionValue(observation_space, action_space, hidden_units) for _ in range(num_critics))
        self.actor = TMPolicy(observation_space, action_space, hidden_units)
        self.critic_output_layers = [c[-1] for c in self.critics]
...
if __name__ == "__main__":
    ...
    Sac_tm = partial(
        ...
        rounds=5,
        steps=100,
        Agent=partial(Agent, device='cpu', Model=partial(Tm_hybrid_1), memory_size=500000, start_training=512, batchsize=256),
