...
from tmrl.core.util import partial, cloudpickle_dump, cloudpickle_load
...
from tmrl.custom.jax.custom_models import NNXSquashedGaussianMLPActor, NNXREDQMLPActorCritic
...
my_run_name = "tutorial_minimal_pendulum_jax"
...
actor_module_cls = partial(NNXSquashedGaussianMLPActor)
...
memory_cls = partial(ArrayNNXMemory,
                     observation_vector_dim=obs_space.shape[0],
                     action_vector_dim = act_space.shape[0],
                     memory_size=1e6,
                     batch_size=32,
                     crc_debug=CRC_DEBUG)
...
training_agent_cls = partial(NNXSACAgent,
...
if __name__ == "__main__":
    my_trainer = Trainer(
        ...
        dump_run_instance_fn=cloudpickle_dump,  # NNX requires cloudpickle serialization rather than pickle
        load_run_instance_fn=cloudpickle_load)  
...
if __name__ == "__main__":
    ...
    rendering_worker = RolloutWorker(
        ...
        actor_module_cls=partial(NNXSquashedGaussianMLPActor),  # TODO: this is not jitted
        sample_compressor=None,
        device="cpu",
        max_samples_per_episode=1000,
        model_path=model_path)
