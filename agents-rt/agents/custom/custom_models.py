...
class TMModuleResnet(Module):
    ...
    def forward(self, x):
        ...
        if self.is_q_network:
            act = x[-1].float()
            h = torch.cat((im, vel, gear, rpm, prev_act, act), dim=1) if self.act_in_obs else torch.cat((im, vel, gear, rpm, act), dim=1)
        else:
            # print(f"DEBUG2 im.shape : {im.shape}, vel.shape : {vel.shape} , prev_act.shape : {prev_act.shape}")
            h = torch.cat((im, vel, gear, rpm, prev_act), dim=1) if self.act_in_obs else torch.cat((im, vel, gear, rpm), dim=1)
