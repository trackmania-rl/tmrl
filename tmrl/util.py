...
def partial(func: Callable[..., Any] = default, *args: Any, **kwargs: Any) -> functools.partial[Any]:
...
def partial_from_args(func: str | Callable[..., Any], kwargs: dict[str, str]):
    # logger.info(func, kwargs)  # useful to visualize the parsing process
...
def git_info(path=None):
    ...
    return {"url": url, "rev": rev, "count": count, "status": status, "desc": desc, "date": commit_date, "message": message}
...
class DelayInterrupt:
    ...
    def on_signal(self, *args):
        logger.info("tmrl.util:DelayInterrupt -- Signal received! args={}", args)
        self.signal_received = True

    def __exit__(self, *args):
        [signal.signal(s, d) for s, d in zip(self.signals, self.default_handlers, strict=False)]
