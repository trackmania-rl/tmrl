"""
Tutorial: a computationally efficient Torch-based TMRL pipeline for real-time robots.
Similar to tuto_minimal_drone.py, but uses the computationally optimized ArrayTorchMemory instead of GenericTorchMemory.
...
from tmrl.custom.custom_memories import ArrayTorchMemory
...
my_run_name = "tutorial_array_drone"
...
env_cls = partial(GenericGymEnv, id="real-time-gym-ts-v1", gym_kwargs={"config": my_rtgym_config}, wrappers=wrappers)
...
memory_cls = partial(ArrayTorchMemory,
                     memory_size=1e6,
                     batch_size=32,
                     crc_debug=CRC_DEBUG)
