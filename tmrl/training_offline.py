...
class TrainingOffline:
    ...
    env_cls: Any = None  # = GenericGymEnv  # dummy environment, used only to retrieve observation and action spaces if needed
    memory_cls: Any = None  # = TorchMemory  # replay memory
    training_agent_cls: Any = None  # = TrainingAgent  # training agent
    ...
    agent_scheduler: Callable[..., Any] | None = None  # if not None, must be of the form f(Agent, epoch), called at the beginning of each epoch
    ...
    def __post_init__(self):
        ...
        self.agent = self.training_agent_cls(observation_space=observation_space, action_space=action_space, device=device)
    ...
    def run_epoch(self, interface):
        ...
        for rnd in range(self.rounds):
            logger.info(f"=== epoch {self.epoch}/{self.epochs} ".ljust(20, "=") + f" round {rnd}/{self.rounds} ".ljust(50, "="))
            ...
            train_time = t3 - t2
            logger.debug(f"round_time:{round_time}, idle_time:{idle_time}, update_buf_time:{update_buf_time}, train_time:{train_time}")
    ...
    def __init__(
        ...
        Args:
            env_cls (type): class of a dummy environment, used only to retrieve observation and action spaces if needed.
                Alternatively, this can be a tuple (observation_space, action_space).
            memory_cls (type): class of the replay memory
            training_agent_cls (type): class of the training agent
            epochs (int): total number of epochs, we save the agent every epoch
            rounds (int): number of rounds per epoch, we generate statistics every round
