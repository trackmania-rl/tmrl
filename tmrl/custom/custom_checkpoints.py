...
def update_memory(run_instance):
    ...
    if run_instance.steps != steps or run_instance.memory.batch_size != batch_size or run_instance.memory.memory_size != memory_size:
        ...
        logger.info(f"Memory updated with steps:{steps}, batch size:{batch_size}, memory size:{memory_size}.")
    return run_instance
...
def update_run_instance(run_instance, training_cls):
    ...
    if ALG_NAME in ["SAC", "REDQSAC"]:
        ...
        if ALG_NAME == "SAC":
            if run_instance.agent.lr_actor != lr_actor:
                ...
                logger.info(f"Actor optimizer reinitialized with new lr: {lr_actor} (old lr: {old}).")
            ...
            if run_instance.agent.lr_critic != lr_critic:
                ...
                logger.info(f"Critic optimizer reinitialized with new lr: {lr_critic} (old lr: {old}).")
        ...
        if ALG_NAME == "REDQSAC":
            ...
            if run_instance.agent.q_updates_per_policy_update != q_updates_per_policy_update:
                ...
                logger.info(f"Q update ratio switched to {q_updates_per_policy_update} (old: {old}).")
    ...
    if run_instance.max_training_steps_per_env_step != max_training_steps_per_env_step:
        ...
        logger.info(f"Max train/env step ratio changed to {max_training_steps_per_env_step} (old: {old}).")
    ...
    if run_instance.start_training != start_training:
        ...
        logger.info(f"Number of environment steps before training changed to {start_training} (old: {old}).")
